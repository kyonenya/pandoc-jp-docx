...
（Word は英数字隣接時は半角で表示するが、日本語隣接時には全角で表示してしまう）

### 全角幅の固定

ローマ数字 `Ⅰ` 〜 `Ⅻ` は必ず全角幅で表示させる。

`イデーンⅠ` → イデーンＩ

### 和欧間アキ

プライム記号 `′` `″` `‴`、半角丸括弧 `(` `)` は隣が日本語のときにアキを補う。

`世界1′において` → 世界 1′ において

`これをP(P)と定義する` → これを P(P) と定義する

（Word は半角英数字と日本語の間に自動でアキを入れるが、記号が挟まると入らなくなる）
...
設定の手順については [PDF 出力の設定](docs/postprocess-pdf.md) を参照のこと。
